...
export function calcBoardSize() {
  ...
  const MOB_DRAW  = _drawerOpen ? 96 : 0;  // action drawer
  const GAPS      = 18;    // padding / gaps

  const availH = vh - MOB_NAV - MOB_HDR - MOB_HUD - MOB_STRIP
               - MOB_ITEMS - MOB_ACTS - MOB_DRAW - GAPS;
...
export function renderObjPanel() {
  ...
  el.innerHTML = `
    ...
    <div class="ob ${G.pCap>=G.captureGoal?'ob-ok':''}"><span class="ob-icon">⚑</span>Cứ điểm (${G.pCap}/${G.captureGoal})</div>
